<template>
  <v-container>
    <div class="card-list-container">
      <CardList :cards="playlistCards" :query="searchQuery" class="playlist-list" @clicked="onClick" />
    </div>

    <transition name="slide">
      <div v-if="selectedPlaylist" class="track-list-container">
        <div class="playlist-info-container" @click="selectedPlaylist = null">
          <div class="playlist-info-background" v-bind:style="{ backgroundImage: 'url(' + selectedPlaylistThumbnail + ')' }"></div>
          ...
          <div class="playlist-info-content">
              ...
              <h2 class="playlist-info-title">
                ...
                <v-dialog v-model="renameDialog" max-width="400px">
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn icon dark v-bind="attrs" v-on="on" small @click="renamedPlaylistName = selectedPlaylist.name">
                      <v-icon size="18">mdi-pencil</v-icon>
                    </v-btn>
                  </template>
                  <v-card>
                    <v-card-title>プレイリスト名を変更</v-card-title>
                    <v-alert v-if="renamePlaylistErrorMessage" :type="renamePlaylistErrorMessage.type" @click="renamePlaylistErrorMessage = null">
                      {{ renamePlaylistErrorMessage.text }}
                    </v-alert>
                    <v-card-text>
                      <v-text-field v-model="renamedPlaylistName" class="text-field-rename"></v-text-field>
                    </v-card-text>
                    <v-card-actions>
                      <v-btn text color="blue" @click="renameSelectedPlaylist">変更</v-btn>
                      <v-btn text color="red" @click="renameDialog = false">キャンセル</v-btn>
                    </v-card-actions>
                  </v-card>
                </v-dialog>
              </h2>
            ...
            <div class="playlist-actions">
              <v-btn icon dark @click.stop="addTracksToQueue">
                <v-icon>mdi-plus</v-icon>
              </v-btn>
              <v-btn icon dark @click.stop="shuffleAndQueue">
                <v-icon>mdi-shuffle-variant</v-icon>
              </v-btn>
              <v-btn icon dark @click.stop="queueAndRepeat">
                <v-icon>mdi-repeat</v-icon>
              </v-btn>
              <v-dialog max-width="300px" v-model="deleteDialog">
                <template v-slot:activator="{ on, attrs }">
                  <v-btn icon dark v-bind="attrs" v-on="on" @click.stop="" class="btn-delete">
                ...
                <v-card>
                  ...
                  <v-card-actions>
                    <v-btn text color="blue" @click="deleteSeledtedPlaylist">はい</v-btn>
        ...
        <TrackList :tracks="selectedPlaylist.tracks" :sort="true" :put="true" pull="clone" :deletable="true" :queueing="true" class="track-list scroll-thin" @deleted="deleteTrack" @changed="updatePlaylistTracks" />
      </div>
    </transition>
  </v-container>
</template>
...
export default Vue.extend({
  ...
  methods: {
    ...
    onClick(playlistCard: Card) {
      this.selectedPlaylist = this.playlists.find((playlist: Playlist) => playlist.name == playlistCard.id) || null
    },
    ...
    renameSelectedPlaylist() {
      ...
      if (this.selectedPlaylist.name !== this.renamedPlaylistName && this.playlists.find((playlist: Playlist) => playlist.name === this.renamedPlaylistName)) {
        this.renamePlaylistErrorMessage = new Message("error", "プレイリストが既に存在します")
        return
      }
