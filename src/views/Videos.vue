<template>
  <v-container class="videos-container">
    <div class="card-list-container">
      <CardList :cards="videoCards" :query="searchQuery" class="video-list" @clicked="onClick" />
    </div>

    <transition name="slide">
      <div v-if="selectedVideo" class="track-list-container">
        <div class="video-info-container" @click="selectedVideo = null">
          <div class="video-info-background" v-bind:style="{ backgroundImage: 'url(' + selectedVideo.getThumbnailURL('hqdefault') + ')' }"></div>
        ...
        <TrackList :tracks="getTracksByVideoId(selectedVideo.id)" :sort="false" :put="false" pull="clone" :queueing="true" class="track-list scroll-thin" />
      </div>
    </transition>
  </v-container>
</template>
...
export default Vue.extend({
  ...
  async created() {
    ...
    this.videoToTracks = new Map(videos.map(video => [video.id, library.tracks.filter(track => track.video.id === video.id)]))
    await Promise.all(videos.map(video => video.fetchVideoInfo()))
    this.videos = videos
    this.videoCards = util
      .sortByKey(this.videos, video => video.publishedAt)
      .reverse()
      .map(video => {
        const keywords: Set<string> = new Set()
        library.tracks
          .filter(track => track.video.id === video.id)
          .map(track => {
