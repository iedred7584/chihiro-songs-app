  ...
  <v-container>
    <div class="list-actions" v-if="listActions">
      <div class="list-actions-left">
        <v-btn v-if="queueing" icon small class="btn-shuffle" @click="addFilteredTracksToQueue">
          <v-icon size="20">mdi-plus</v-icon>
        </v-btn>

        <v-btn icon small class="btn-shuffle" @click="shuffle">
          <v-icon size="20">mdi-shuffle-variant</v-icon>
        </v-btn>

        <v-btn icon small class="btn-repeat" @click="repeat">
      ...
      <div class="list-actions-right">
        <v-btn icon small class="btn-repeat" @click="loadedTracks.reverse()">
    ...
    <div class="track-list-container scroll-thin">
      <draggable :list="loadedTracks" tag="ul" animation="200" :group="{ name: 'tracks', put: put, pull: pull }" :clone="cloneTrack" :sort="sort" handle=".handle" @change="onChange" class="track-list">
        <li v-for="track in loadedTracks" v-bind:key="track.uuid" class="track" v-bind:class="{ nowPlayingTrack: isNowPlaying(track) }" v-bind:style="{ display: track.match(updatedQuery) ? 'block' : 'none' }">
          <Track :track="track" :deletable="deletable" :nowPlayingId="nowPlayingId" :queueing="queueing" @clicked="onClick" @deleted="deleteTrack" />
        </li>
      </draggable>
    </div>
  </v-container>
...
export default Vue.extend({
  name: "TrackList",
  props: ["tracks", "put", "pull", "sort", "queueing", "deletable", "nowPlayingId", "query", "listActions"],
  components: {
    draggable,
  ...
  methods: {
    ...
    onChange() {
      this.$emit(
        "changed",
        this.filteredTracks.map(track => track)
      )
    },
    ...
    deleteTrack(targetTrack: Track) {
      this.loadedTracks = this.loadedTracks.filter((track: Track) => track.uuid !== targetTrack.uuid)
