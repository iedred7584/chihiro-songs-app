<template>
  <v-container class="card-list-container">
    <div class="card-list scroll-thin">
      <div v-for="card in cards" v-bind:key="card.id" class="card-list-item" v-bind:style="{ display: matchQuery(card) ? 'block' : 'none' }">
        <Card :card="card" @clicked="onClick" class="card" />
      </div>
    </div>
  </v-container>
</template>
...
export default Vue.extend({
  ...
  methods: {
    ...
    matchQuery(card: Card): boolean {
      const target = [card.title, card.subtitle, card.metadata].join("  ").toLowerCase()
      let match = true
