<template>
  <v-container>
    <v-card flat tile :ripple="false" @click="onClick" v-bind:class="{ nowplayingtrack: nowPlaying }" class="track-card">
      <div class="d-flex flex-no-wrap justify-space-between track-content">
        ...
        <div class="track-info">
          <v-btn right icon width="25" height="25" :color="track.isFavorite ? 'red' : ''" @click.stop="toggleFavorite" class="btn-favorite">
            <v-icon size="17">{{ track.isFavorite ? "mdi-heart" : "mdi-heart-outline" }}</v-icon>
          </v-btn>

          <v-card-title class="track-info-item track-title" v-text="track.title + ' / ' + track.artist"></v-card-title>

          <v-card-subtitle class="track-info-item track-artist" v-text="track.singer"></v-card-subtitle>

          <v-card-text class="track-info-item video-info">
            <div class="track-info-item">
              <a :href="track.video.getURL()" target="_blank" @click.stop="">
                <v-icon size="15" class="icon-youtube">mdi-youtube</v-icon>
              </a>
              <span>{{ track.video.title }}</span>
            </div>
            <div class="track-info-item">
              <a :href="track.hasVideoInfo() ? track.video.channel.url : ''" target="_blank" @click.stop="">
        ...
        <div class="track-thumbnail ma-3">
          <v-img :src="track.video.getThumbnailURL('mqdefault')"></v-img>
          <div class="track-actions">
            <v-btn v-if="queueing" dark right icon @click.stop="addTrackToQueue(track)">
              <v-icon size="18">mdi-plus</v-icon>
            </v-btn>

            <v-dialog max-width="450" scrollable v-model="playlistDialog">
              <template v-slot:activator="{ on, attrs }">
                <v-btn dark right icon @click.stop="" v-bind="attrs" v-on="on">
      ...
      <div v-if="deletable" class="btn-delete-track">
        <v-btn dark right icon width="20" height="20" @click.stop="onDelete(track)">
