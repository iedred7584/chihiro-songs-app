  ...
  <v-container>
    <div class="queue-actions">
      <div class="queue-actions-left">
        ...
        <v-btn icon small class="btn-loop" @click="togglePlayerRepeat">
          <v-icon size="20"> {{ repeatIcon }} </v-icon>
        </v-btn>
      </div>

      <div class="queue-actions-center">
        <span class="num-queued-tracks" v-on:click="scrollPlayingTrackIntoView"> {{ playingTrackIndex }} / {{ queuedTracks.length }} </span>
      </div>

      <div class="queue-actions-right">
        <v-btn icon small class="btn-delete" @click="clean">
    ...
    <div class="queue-list">
      ...
      <TrackList :tracks="queuedTracks" :deletable="true" :queueing="false" :sort="true" pull="clone" nowPlayingId="uuid" @changed="onChange" @deleted="onDeleted" class="queued-tracks scroll-thin" />
    </div>
  </v-container>
</template>
...
export default Vue.extend({
  ...
  methods: {
    ...
    onDeleted(targetTrack: Track) {
      this.setQueue(this.queuedTracks.filter((track: Track) => track.uuid !== targetTrack.uuid))
    },
    clean() {
      this.setQueue([])
    },
    ...
    shuffle() {
      ...
      if (this.playingTrack && tracks.map(track => track.uuid).indexOf(this.playingTrack.uuid) >= 0) {
