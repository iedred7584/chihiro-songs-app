<template>
  <v-container>
    <div class="video-screen">
      <youtube ref="youtube" video-id="" :player-vars="playerVars" :fitParent="true" :resize="true" @playing="playing" @paused="paused" @ended="ended" class="youtube-player"></youtube>
    </div>

    <div class="playing-track-info">
      <v-btn icon small :color="playingTrack && playingTrack.isFavorite ? 'red' : ''" @click="toggleFavorite" class="btn-favorite">
      ...
      <div class="playing-track-details">
        ...
        <div class="playing-track-singer" target="_blank">
      ...
      <div class="volume-control">
        <div class="volume-slider">
          <v-slider max="100" min="0" dark v-model="volume"></v-slider>
        </div>
        <v-btn icon small class="btn-volume" @click="toggleMute">
    ...
    <div class="video-controller">
      <div class="seekbar-container">
        ...
        <v-slider v-model="seekbarValue" @start="seekStart" @end="seekEnd" @click="seekEnd" min="0" :max="seekbarMax" color="accent" track-color="accent lighten-1" class="seekbar"></v-slider>
      ...
      <div class="player-control-buttons">
        ...
        <div class="btn-play-pause">
          <v-btn v-if="playerState !== 1" @click="playVideo" icon large>
            <v-icon size="50">mdi-play</v-icon>
          </v-btn>
          <v-btn v-else @click="pauseVideo" icon large>
            <v-icon size="50">mdi-pause</v-icon>
          </v-btn>
        </div>
        <div class="btn-skip-next">
          <v-btn icon rounded @click="playNext()">
...
import { YoutubeIframe, YoutubeIframePlayer, YoutubePlayerState } from "youtube-iframe-api"
...
export default Vue.extend({
  ...
  computed: {
    ...
    volumeIcon(): string {
      ...
      return this.playerVolume === 0 ? "mdi-volume-mute" : this.playerVolume < 20 ? "mdi-volume-low" : this.playerVolume < 80 ? "mdi-volume-medium" : "mdi-volume-high"
    },
  ...
  async mounted() {
    ...
    this.player.addEventListener("onStateChange", (state: YoutubePlayerState) => {
