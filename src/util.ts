...
export function secondsToTime(t: number): string {
  ...
  if (hours === 0) return [zeroPadding(minutes, 2), zeroPadding(seconds, 2)].join(":")
  return [zeroPadding(hours, 2), zeroPadding(minutes, 2), zeroPadding(seconds, 2)].join(":")
}
...
export function sortTracksByDate(tracks: Track[], reverse = false): Track[] {
  return tracks
    .map(track => track)
    .sort((a, b) => {
      if (a.publishedAt < b.publishedAt) return reverse ? 1 : -1
      if (a.publishedAt > b.publishedAt) return reverse ? -1 : 1
